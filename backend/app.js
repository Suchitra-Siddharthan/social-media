...
app.get('/api/feed', async (req, res) => {
  ...
  try {
    connection = await getConnection();  // Use your existing db connection helper

    // Execute the SQL query with the two functions
    const result = await connection.execute(`
      SELECT 
        p.PostID, 
        p.UserID, 
    ...
    const posts = result.rows.map(row => {
      ...
      result.metaData.forEach((col, i) => {
        post[col.name.toLowerCase()] = row[i];
      });
      return post;
    });
...
app.post('/api/posts/:postid/comments', async (req, res) => {
  ...
  try {
    ...
    const result = await connection.execute(
      `INSERT INTO POSTCOMMENT (POSTID, USERID, COMMENTTEXT, COMMENTDATE)
       VALUES (:postid, :userid, :commenttext, SYSTIMESTAMP)
